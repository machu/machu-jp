...
$page_title = '人狼BBS まとめサイト';

// Specify PukiWiki URL (default: auto)
// $script = 'http://new.wolfbbs.jp/';
...
// CHANGE THIS
$adminpass = '{x-php-md5}7414000183b75c5c0e20fdd6626c96ab'; // md5('pass')
...
$maxage = 12; // Stock latest N backups
...
$auto_template_rules = array(
  '^[A-Z]?(\d+)村$' => 'VillageTemplate'
);

/////////////////////////////////////////////////
// Notice for edit form
$edit_notice = <<<EOD
 <div class="edit_notice">
  <p>本サイトに書き込んだ文書には、 <a href="http://creativecommons.org/licenses/by-nc-sa/2.0/jp/deed.ja">CCPL-by-nc-sa ライセンス</a> が適用されます。(<a href="/?ServicePolicy">ServicePolicy</a> を参照)</p>
  <p>トラブル・悪戯対策のため、IPアドレスを記録しています。(<a href="/?PrivacyPolicy">PrivacyPolicy</a> を参照)</p>
 </div>
EOD;
...
//define('WHITELIST', "/(\.jp)|(yahoobb\d+\.bbtec\.net)$/i");
define('WHITELIST', "/(\.jp)|(yahoobb\d+\.bbtec\.net)|(\.il24\.net)|(\.\d?iij\.net)|(kddi\.com)|(net3-tv\.net)|(george24\.com)|(i-products\.net)|(jfe-gr\.net)|(\.ph\.ph\.cox\.net)|(\.office\.freebit\.net)|(\.asyst-shinko\.com)|(\.co\.comcast\.net)|(\.zero-isp\.net)|(\.bitcat\.net)|(\.server.ntli\.net)|(\.kddi\.fr)|(\.qala\.com\.sg)|(\.g-trek\.com)|(\.interbusiness\.it)|(\.mediatti\.net)|(\.comcast\.net)|()$/i");
...
function check_commentFilter($msg) {
  // if(preg_match("/^[\[\]\|<>a-zA-Z0-9_:!\/\"'\-=@#% .,?]+$/i", $msg)) {
  if(preg_match("/^[!-~\n ]+$/", $msg)) {
    die_message('日本語を含まないコメントは書き込みできません。');
  }
  if (preg_match("/<a href=/", $msg) || preg_match("/会えますか/", $msg) || preg_match("/即アポ/", $msg)) {
    die_message('コメントがSPAM判定されました。書き込みできません。');
  }
}

// 2008-10-10 Apache2.2でのmod_rewrite対策（QUERY_STRINGが勝手にURLデコードされる問題へ対処）
