...
function plugin_counter_get_count($page)
{
	...
	if ($counters[$page]['date'] != $default['date']) {
		// New day
		$modify = TRUE;
		$is_yesterday = ($counters[$page]['date'] == get_date('Y/m/d', strtotime('yesterday', time())));
		$counters[$page]['ip']        = $_SERVER['HTTP_X_FORWARDED_FOR'];
		$counters[$page]['date']      = $default['date'];
		$counters[$page]['yesterday'] = $is_yesterday ? $counters[$page]['today'] : 0;
		$counters[$page]['today']     = 1;
		$counters[$page]['total']++;

	} else if ($counters[$page]['ip'] != $_SERVER['HTTP_X_FORWARDED_FOR']) {
		// Not the same host
		$modify = TRUE;
		$counters[$page]['ip']        = $_SERVER['HTTP_X_FORWARDED_FOR'];
		$counters[$page]['today']++;
